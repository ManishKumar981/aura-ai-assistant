  ...
  const summary = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select("chief_complaint, subjective, objective, assessment, plan, overview, follow_up, generated_by")
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3 rounded-lg border border-border bg-muted/60 p-4">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          anything not mentioned is shown as “Not reported”. No diagnosis is confirmed and no medication is prescribed.
        </p>
      </div>

      <section className="clinical-panel p-6">
        <h2 className="flex items-center gap-2 text-lg font-semibold">
          <FileText className="size-4 text-muted-foreground" /> Summary
        </h2>
        ...
        {summary.data && (
          <div className="mt-4 space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Plan / recommendations" value={summary.data.plan} />
            </div>
          </div>
        )}
      </section>
